...
def buildDecisionTree(data, root, remainingFeatures, depth):
  ...
  if(len(remainingFeatures) == 0):
    return getLabelWithMajority(data,root)
...
calculateInformationGain(data,index))
  ...
  for dataSubset in splitData(data, bestFeature):
    ...
    buildDecisionTree(dataSubset, child, remainingFeatures - set([bestFeature]),depth+1)
  return root

def run_decision_tree(data):
  return buildDecisionTree(data, DecisionTree(), set(range(len(data[0][0]))),0)
...
if __name__ == "__main__":
  print "---- Decision Tree Classifier with majority voting and pruning ----"
